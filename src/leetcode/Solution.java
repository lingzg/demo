...
public class Solution {
    ...
    private int getDeep(ListNode node) {
        int deep = 0;
        while (node != null) {
            deep++;
            node = node.next;
    ...
    public String longestPalindrome(String s) {
        ...
        for(int i=0;i<s.length()-1;i++){
            int[] a1 = getPalindrome(i,i,s);
            int[] a2 = getPalindrome(i,i+1,s);
            if(a1[0]>ans[0]){
                ans = a1;
            }
            if(a2[0]>ans[0]){
    ...
    private int[] getPalindrome(int l,int r,String s){
        int flag = 0;;
        ...
        return new int[]{r-l+1, l+flag, r-flag};
    }
    ...
    public String convert(String s, int numRows) {
        ...
        //解法二：step = [(numRows-1-i)*2 , i*2]
        StringBuilder sb = new StringBuilder();
        for(int i=0;i<numRows;i++){
            int step  = (numRows-1)*2;
            int[] steps = {(numRows-1-i)*2, i*2};
            int k=0;
    ...
    public int reverse(int x) {
        int n = 0;
        if(x==Integer.MIN_VALUE){//-2147483648
            return n;
        }
        int t = Integer.MAX_VALUE/10;//214748364
        while(true){
            n = n*10+x%10;
            x/=10;
            if(x==0){
     ...
     * 
     * 提示： 本题中的空白字符只包括空格字符 ' ' 。 假设我们的环境只能存储 32 位大小的有符号整数，那么其数值范围为 [−231,  231 −
     * 1]。如果数值超过这个范围，请返回  INT_MAX (231 − 1) 或 INT_MIN (−231) 。 来源：力扣（LeetCode）
    ...
    public int myAtoi(String str) {
        ...
        return Integer.parseInt((symbol < 0 ? "-" : "") + numStr);
    }
    ...
    /**
     * 11. 盛最多水的容器 
     * 给你 n 个非负整数 a1，a2，...，an，每个数代表坐标中的一个点 (i, ai) 。在坐标内画 n条垂直线，垂直线 i 的两个端点分别为 (i, ai) 和 (i, 0)。 
    ...
    public int findLUSlength(String[] strs) {
        Map<String, Integer> maxLenStr = new HashMap<String, Integer>();
        int maxLen = 0;
        for (int i = 0; i < strs.length; i++) {
            if (strs[i].length() > maxLen) {
                maxLenStr.clear();
                maxLenStr.put(strs[i], 1);
                maxLen = strs[i].length();
            }
            if (strs[i].length() == maxLen) {
                Integer len = maxLenStr.get(strs[i]);
                maxLenStr.put(strs[i], len == null ? 0 : len + 1);
            }
        }
        boolean flag = true;
        for (Integer i : maxLenStr.values()) {
            if (i > 1) {
                flag = false;
                break;
            }
        ...
        for (int i = 0; i < strs.length; i++) {
            if (maxLenStr.containsKey(strs[i])) {
                continue;
            }
        }
        return -1;
...
//        System.out.println(e-s);
    ...
    public List<String> letterCombinations(String digits) {
        ...
        backtrack(0,output, digits,letters,res);
        return res;
    }
    
    private void backtrack(int index,StringBuilder output,String digits,String[] letters,List<String> res){
        ...
        for(int i=0;i<letter.length();i++){
            ...
            backtrack(index+1,output, digits,letters,res);
    ...
    public List<List<Integer>> fourSum(int[] nums, int target) {
        ...
        for (int i = 0; i < len; i++) {
            ...
            for(int n = i+1;n<len;n++){
            	...
            	int j=n+1,k=len;
            	while (j<k) {
            		int sum = nums[i] + nums[n] + nums[j] + nums[k];
            		if (sum == target) {
     ...
     * 算法：使用栈，遇到前括号进栈，遇到后括号出栈，检查出栈的元素和当前元素是否匹配
     * @param s
     * @return
     */
     ...
     * 回溯算法
     * 思路一：构建一个数组，前n个元素是左括号，后n个元素是右括号，做无重复解的回溯
     * 思路二：如果左括号数量不大于 n，我们可以放一个左括号。如果右括号数量小于左括号的数量，我们可以放一个右括号。
     * @param n
     * @return
     */
    public List<String> generateParenthesis(int n) {
        ...
        for(int i=0;i<n;i++){ // 左括号放在前n个位置，右括号放在后n个位置，相当于做了排序
        	arr[i]='(';
        	arr[n+i]=')';
        }
        StringBuilder output = new StringBuilder();
        boolean[] used = new boolean[n*2];
        dfs(arr, output, used, res);
        return res;
    }
    
    private void dfs(char[] arr,StringBuilder output,boolean[] used,List<String> res){
        ...
        for(int i=0;i<arr.length;i++){
    		...
    		if(i>=arr.length/2&&!used[i-arr.length/2]){ // 作括号没有使用，右括号不能使用
    			continue;
    		}
    		if(i>0 && arr[i]==arr[i-1]&&!used[i-1]){ // 重复元素减枝
    			continue;
    		}
    		used[i]=true;
    		output.append(arr[i]);
    ...
    public List<String> generateParenthesis2(int n) {
        ...
        StringBuilder output = new StringBuilder();
        dfs2(n, output, 0, 0, res);
        return res;
    }
    
    private void dfs2(int n,StringBuilder output,int open, int close,List<String> res){
        ...
        if(open<n){
        	output.append("(");
        	dfs2(n, output, open+1, close, res);
        	output.deleteCharAt(output.length()-1);
        }
        if(close<open){
        	output.append(")");
        	dfs2(n, output, open, close+1, res);
        	output.deleteCharAt(output.length()-1);
        }
    }
     ...
     * 
     * 二分查找第一个>=target的元素位置
     * 边界情况：target>nums中最大元素，返回nums.length；target<nums中最小元素，返回0
     * 
     * @param nums 升序数组
     * @param target 目标元素
     * @return
    ...
    public boolean isValidSudoku(char[][] board) {
        for (int i = 0; i < 9; i++) {
            ...
            for (int j = 0; j < 9; j++) {
                ...
                char g = board[(i / 3) * 3 + j / 3][(i % 3) * 3 + j % 3];
                if (g != '.') {
                    if (!grid.contains(g)) {
                        grid.add(g);
                    } else {
    ...
    public String countAndSay(int n) {
        ...
        for (int i = 1; i < n; i++) {
            ...
            s = sb.append(count).append(num).toString();
        }
        return s;
    }
    ...
    public int firstMissingPositive(int[] nums) {
        ...
        for (int i = 0; i < n; i++) {
            int x = nums[i];
            while (x >= 1 && x <= n) {// if x∈[1,n] 把x交换到x-1位置
                if (x == nums[x - 1]) {
                    break;
                }
                nums[i] = nums[x - 1];
    ...
    /**
     * 43. 字符串相乘 
     * 给定两个以字符串形式表示的非负整数 num1 和 num2，返回 num1 和 num2的乘积，它们的乘积也表示为字符串形式。
     * 
     * @param num1
     * @param num2
     * @return
    ...
    public List<List<Integer>> permute(int[] nums) {
        ...
        backtrack(output, used, nums, res);
        return res;
    }

    private void backtrack(Stack<Integer> output, boolean[] used, int[] nums, List<List<Integer>> res) {
     ...
     * 规律：i,j -> j,n-1-i
     * 方法：把矩阵看成(n+1)/2个由数字组成的正方形层层嵌套的图像，
     * 从最外层的正方形 [(0,0)->(0,n-1)->(n-1,n-1)->(n-1,0)->(0,0)]开始，
     * 把正方形上各个数字交换到旋转后的位置，再逐个交换内层的正方形
     * 
     * @param matrix
     */
    public void rotate(int[][] matrix) {
        int n = matrix.length;
        for (int x = n, j = 0; x > 1; x -= 2) { // 内层正方形的长度减2
            for (int i = 0; i < x - 1; i++) { //遍历正方形上的各个数字
                int t = matrix[j][i + j];
...
//        System.out.println(s.myPow(2.3,11));
//        System.out.println(s.longestPalindrome("aaabaabaa"));
//        System.out.println(s.convert("LEETCODEISHIRING", 3));
//        System.out.println(s.reverse(-2147483641));//2147483647
//        System.out.println(s.threeSumClosest(new int[]{-1,2,1,-4}, 1));
//        List<List<Integer>> list = s.fourSum(new int[]{1,-2,-5,-4,-3,3,3,5}, -11);
//        System.out.println(list);
